...
Network Address Translation (NAT) dibutuhkan untuk memungkinkan banyak perangkat dalam jaringan lokal (privat) mengakses internet menggunakan satu alamat IP publik.
...
1. Static NAT (1:1)
Jenis NAT ini memetakan satu alamat IP privat secara permanen ke satu alamat IP publik. Hubungannya bersifat dua arah (timbal balik).
Cara Kerja: Alamat IP lokal 192.168.1.10 akan selalu diterjemahkan menjadi IP publik 202.10.20.30.
Fungsi Utama: Digunakan untuk perangkat lokal yang harus bisa diakses secara konstan dari luar internet, contohnya seperti server web, server email, atau server FTP perusahaan.
2. Dynamic NAT (Pool)
Jenis NAT ini memetakan IP privat ke IP publik secara otomatis dari sekumpulan (pool) IP publik yang tersedia, namun tetap berbasis satu-ke-satu saat terhubung.
Cara Kerja: Ketika komputer 192.168.1.10 ingin ke internet, router meminjamkan IP publik 202.10.20.30. Saat komputer .11 menyusul, router memberikan IP .31, dan seterusnya. Hubungan ini tidak permanen; jika sesi selesai, IP publik akan dilepas kembali ke pool.
Kelemahan: Jika jumlah IP publik di pool habis terpakai, komputer berikutnya yang ingin mengakses internet harus mengantre sampai ada IP publik yang kosong.
3. PAT / NAT Overload (Many:1)
Jenis NAT yang paling populer karena memungkinkan banyak IP privat menggunakan satu IP publik yang sama secara bersamaan dengan memanfaatkan nomor port unik.
Cara Kerja: Komputer .10, .11, dan .12 semuanya keluar ke internet menggunakan IP publik yang persis sama (202.10.20.30). Agar paket data tidak tertukar saat kembali, router memberikan penanda berupa nomor port yang berbeda di belakang IP publik (misalnya port :12345, :12346, dan :12347).
Fungsi Utama: Metode ini digunakan pada jaringan rumah dan kantor kecil (SOHO) karena sangat menghemat biaya penyewaan alamat IP publik dari ISP.
